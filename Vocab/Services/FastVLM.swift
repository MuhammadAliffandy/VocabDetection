        ...
        case .messages(let raw):
            messages = raw.map { message in
                ...
                for (key, value) in message {
                    converted[key] = value as? String ?? "\(value)"
                }
                return converted
            }
        case .chat(let chat):
            messages = chat.map { ["role": $0.role.rawValue, "content": $0.content] }
        }
        if messages.isEmpty {
...
public class FastVLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    static public func register(modelFactory: VLMModelFactory) async {
        await modelFactory.typeRegistry.registerModelType("llava_qwen2") { configurationData in
            let configuration = try JSONDecoder().decode(
                FastVLMConfiguration.self, from: configurationData)
            return FastVLM(configuration)
        }

        await modelFactory.processorRegistry.registerProcessorType("LlavaProcessor") {
            configurationData, tokenizer in
            let configuration = try JSONDecoder().decode(
                FastVLMPreProcessorConfiguration.self, from: configurationData)
